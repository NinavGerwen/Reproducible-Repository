  ...
  for(b in levels(sample_size)){
      ...
      temp_k <- as.numeric(as.character(a))
        ...
        ## current condition
          ...
          tli_yang[j] <- TLI_Cai_Calc(l_0, temp_data, aggr_data, temp_n, temp_k)
          
          cfi_yang[j] <- CFI_Cai_Calc(l_0, temp_data, aggr_data, temp_n, temp_k)
...
for(a in levels(test_length)){
  ...
  for(b in levels(sample_size)){
      ...
      alt_results <- rbind(alt_results, c(a, b, mean(tli_values,
                                                     na.rm = TRUE), sd(tli_values, na.rm = TRUE),
                                          mean(cfi_values, na.rm = TRUE), sd(cfi_values, na.rm = TRUE),
                                          mean(tli_yang, na.rm = TRUE)))
